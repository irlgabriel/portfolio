    ...
    >
      ...
      <Box ref={ref} className={`${classes.root} slide-up`} style={{minHeight: '600px'}}>
        ...
        <Box>
          ...
          <InfoSection color='lightblue' left order={2} count={4} title={'September 2020'} subtitle={'Firebase and other SaaS'} info="Started playing with google's firebase toolset - firestore, authentication and hosting - to build simple apps that would further my knowledge with react." />          
        </Box>
        <Box>
          <CurvedArrow count={4} order={3} color='orange' direction='right'/>
          <InfoSection color='orange' right order={3} count={4} title={'November 2020'} subtitle={'Node and Express'} info='Finally delved into backend stuff for real this time. I was not gonna count ruby on rails - although I was able to build some nice stuff with it, the whole thing with route helpers, form helpers, activestorage and such was quite confusing to me back then. After getting familiar and building things with express I now grew to appreciate the things rails does under the hood.' />          
        </Box>
        <Box>
          <CurvedArrow count={4} order={4} color='green' direction='left'/>
          <InfoSection color='green' left order={4 } count={4} title={'Now'} subtitle={'Getting Hired @ LinkedIn'} info="Literally building this 'career path' section right now to make the portfolio more appealing, along with applying to new jobs on LinkedIn on a daily basis." />          
        </Box>
      </Box>
    </CSSTransition>
  )
